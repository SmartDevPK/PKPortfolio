...
<body>
      ...
      <section data-panel="customers" style="display:none;">
        <div class="grid">
          <div class="card">
            ...
            <div>
              <label for="custMsg">Message</label>
              <textarea id="custMsg" placeholder="Message..."></textarea>
            </div>
            <div class="actions">
              <button id="saveCustomer" class="btn primary">Save Message</button>
              <button id="resetCustomer" class="btn muted">Reset</button>
            </div>
          </div>
          <div class="card">
    ...
    <script>
      ...
      function renderCustomers() {
        ...
        customersList.appendChild(loading);

        fetch('api/messages_list.php')
          .then(r => r.ok ? r.json() : Promise.reject())
          .then(items => {
            customersList.innerHTML = '';
            if (!Array.isArray(items) || !items.length) { customersList.innerHTML = `<div class="empty">No messages yet</div>`; return; }
            items.forEach(c => {
              const el = document.createElement('div');
              el.className = 'item';
              el.innerHTML = `
              ...
              `;
              el.querySelector('button').addEventListener('click', () => {
                fetch('api/messages_delete.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ _method: 'DELETE', id: c.id }) })
          ...
          .catch(() => { customersList.innerHTML = `<div class="empty">Failed to load messages</div>`; });
      }
      ...
      document.addEventListener('keydown', (e) => {
        if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') {
          ...
          if ($('.tab.active')?.getAttribute('data-tab') === 'customers') $('#saveCustomer').click();
        }
      });
      ...
      $('#saveCustomer').addEventListener('click', () => {
        ...
        const message = custMsg.value.trim();
        ...
        fetch('api/messages_create.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ name, email, message }) })
          .then(r => { if (!r.ok) throw new Error(); return r.text(); })
          .then(() => { renderCustomers(); custName.value=''; custEmail.value=''; custMsg.value=''; })
          .catch(() => alert('Failed to save message'));
      });

      $('#resetCustomer').addEventListener('click', () => { custName.value=''; custEmail.value=''; custMsg.value=''; });
      ...
      $('#clearCustomers').addEventListener('click', () => {
        ...
        fetch('api/messages_delete.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ _method: 'DELETE', id: 'all' }) })
          .then(() => renderCustomers())
